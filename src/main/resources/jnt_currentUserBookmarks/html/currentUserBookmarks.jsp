...
<c:url var="mainResourceUrl" value="${url.mainResource}"/>
<c:set var="ps" value="?pagerUrl=${mainResourceUrl}"/>
...
<div id="bookmarkList${user.identifier}">
	<template:initPager totalSize="${moduleMap.end}"
						pageSize="${currentNode.properties['numberOfBookmarksPerPage'].string}"
						id="${renderContext.mainResource.node.identifier}"/>
    ...
    <c:if test="${currentResource.workspace eq 'default'}">
        <script type="text/javascript">
            ...
            value="${url.baseLive}${currentNode.path}.html.ajax"/>');
        </script>
    </c:if>
	...
	<c:if test="${currentResource.workspace ne 'default'}">
		...
		<c:if test="${moduleMap.listTotalSize ne 0}">
			<ul class="userMyBookmarksList" id="${currentNode.UUID}">
				<c:forEach items="${moduleMap.currentList}" var="bookmark" varStatus="status" begin="${moduleMap.begin}"
						   end="${moduleMap.end}">
					<li>
						<jcr:nodeProperty node="${bookmark}" name="jcr:title" var="title"/>
						<jcr:node var="myNode" path="${bookmark.path}"/>
						<i class="fa fa-bookmark"
						   aria-hidden="true"></i><a class="userMyBookmarksListIcon"
													 href="${bookmark.properties['url'].string}">${bookmark.properties['jcr:title'].string}</a>
